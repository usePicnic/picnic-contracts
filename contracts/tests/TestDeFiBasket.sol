...
contract TestDeFiBasket is IDeFiBasket, ERC721, Ownable {
    ...
    */
    ...
    function _depositToWallet(
        ...
        for (uint16 i = 0; i < inputs.tokens.length; i++) {
            ...
            IERC20(inputs.tokens[i]).safeTransferFrom(
                ownerOf(nftId),
                defibasketContractOwner,
                defibasketFee
            );
            ...
            IERC20(inputs.tokens[i]).safeTransferFrom(
                ownerOf(nftId),
                walletAddress,
                inputs.amounts[i] - defibasketFee
            );
        }
    }
    ...
    /**
      ...
      *
      * @dev This is where the magic happens. Bridges interact with delegate calls to enable DeFi Basket to interact
      * with a wide and expanding variety of protocols.
      *
      * @param nftId NFT Id
